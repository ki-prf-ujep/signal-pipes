...
(p.sigcontainer(["markers"])
 | ChannelSelect([0, 1, 2])
 | Tee (SubSample(0.0, 30.0) | PResample(new_freq=10)
        | Hdf5Saver("/tmp/posun.h5")
        | Plot(annot_specs=[None, "markers"], file="/tmp/{0.id}.svg"))
 | SubSample(0.0, np.timedelta64(1, "m"))
 ...
 | MarkerSplitter("markers", left_outer_segments=True)
    | FeatureExtraction(target="local_features")
    | Plot(file="/tmp/{0.id}.svg")
    | Hdf5Saver("/tmp/{0.id}.h5")
